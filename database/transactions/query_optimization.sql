...
SELECT
    ...
    SUM(CASE WHEN p.status = 'available' THEN 1 ELSE 0 END) AS dang_ban,
    SUM(CASE WHEN p.status = 'pending'   THEN 1 ELSE 0 END) AS cho_duyet,
    SUM(CASE WHEN p.status = 'sold'      THEN 1 ELSE 0 END) AS da_ban
FROM categories c
