        ...
        <div class="glass-card p-8 rounded-xl border border-outline-variant/40 shadow-sm w-full max-w-md">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="password" id="password" autocomplete="new-password" class="w-full px-4 py-2.5 bg-white border border-outline-variant/40 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all" placeholder="Tối thiểu 8 ký tự...">
                </div>
                <div>
                    <label class="block text-label-sm font-medium text-on-surface-variant mb-1.5">Xác nhận mật khẩu</label>
                    <input type="password" id="confirmPassword" autocomplete="new-password" class="w-full px-4 py-2.5 bg-white border border-outline-variant/40 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all" placeholder="Nhập lại mật khẩu...">
                </div>
                <button onclick="register()" id="registerBtn" class="w-full bg-primary text-on-primary py-3.5 rounded-full font-label-md hover:opacity-90 active:scale-95 transition-all shadow-sm mt-4 uppercase tracking-wide">Đăng ký</button>
            </div>
    ...
    <script>
        async function register() {
            ...
            if (password.length < 8) {
                showAlert("Mật khẩu ngắn", "Mật khẩu phải chứa ít nhất 8 ký tự.", "warning");
                return;
            }

            if (password !== confirmPassword) {
                showAlert("Không trùng khớp", "Mật khẩu xác nhận không khớp.", "warning");
                return;
            }
            ...
            try {
                ...
                let res = await fetch("/backend/public/index.php/api/auth/register", {
                    method: "POST",
                    headers: {
