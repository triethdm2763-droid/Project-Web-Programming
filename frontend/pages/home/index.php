...
<body class="bg-surface font-body-md text-on-surface min-h-screen flex flex-col">
    ...
    <main class="max-w-container-max mx-auto px-gutter py-8 flex-grow w-full space-y-12">
        ...
        <div class="relative w-full rounded-[24px] overflow-hidden shadow-sm h-[200px] md:h-[400px]">
            ...
            <div class="absolute inset-0 w-full h-full">
                ...
                <img src="<?= app_url('/backend/uploads/products/banner1.jpg') ?>"
                ...
                <img src="<?= app_url('/backend/uploads/products/banner2.jpg') ?>"
                ...
                <img src="<?= app_url('/backend/uploads/products/banner3.jpg') ?>"
                    class="banner-slide absolute inset-0 w-full h-full object-cover object-center transition-opacity duration-1000 ease-in-out opacity-0">
            ...
            <div class="relative z-20 h-full flex flex-col justify-center px-6 md:px-10 max-w-xl space-y-4 text-white">
                ...
                <a href="<?= app_url('/frontend/pages/products/category.php') ?>" class="bg-white text-primary font-semibold px-6 py-2.5 rounded-full text-[14px] w-max hover:bg-opacity-90 active:scale-95 transition-all text-center shadow">
                    Khám Phá Ngay
                </a>
            </div>
    ...
    <script src="<?= app_url('/frontend/assets/js/products.js') ?>?v=20260626-4"></script>
    ...
    <script>
        ...
        async function loadProducts(page = 1) {
            ...
            try {
                let res = await fetch(window.appUrl(`/backend/public/index.php/api/products?limit=${homeLimit}&page=${page}`));
                ...
                if (products && products.length > 0) {
                    const productsHtml = products.map(row => {
                        ...
                        return `
                    <a href="${window.appUrl(`/frontend/pages/products/detail.php?id=${row.ID}`)}" class="bg-white border border-slate-100 rounded-2xl overflow-hidden hover:shadow-[0_8px_30px_rgba(0,0,0,0.05)] hover:-translate-y-1 transition-all duration-300 flex flex-col group">
                        <div class="aspect-square bg-slate-50 flex items-center justify-center relative overflow-hidden shrink-0">
                            ${badgeHtml}
                            <img src="${(row.Image && (row.Image.startsWith('http://') || row.Image.startsWith('https://'))) ? escapeHtml(row.Image) : window.appUrl('/backend/uploads/products/' + escapeHtml(row.Image || 'placeholder.png'))}" alt="${escapeHtml(row.Name)}" onerror="this.src=window.appUrl('/frontend/assets/images/placeholder.png')" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                        </div>
