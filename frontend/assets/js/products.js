...
document.addEventListener("DOMContentLoaded", function () {
    ...
    btnCreatePost.addEventListener("click", function (e) {
      ...
      if (isLoggedIn) {
        ...
        window.location.href =
          "/Project-Web-Programming/frontend/pages/seller/my-store.php";
      } else {
        // ĐÃ SỬA: Thay alert bằng showToast dạng cảnh báo (warning)
        showToast(
          "Bạn cần phải đăng nhập tài khoản trước khi thực hiện chức năng đăng tin thanh lý đồ cũ!",
          "warning",
        );
        setTimeout(() => {
          window.location.href =
  ...
  const searchInput = document.querySelector(
    'input[placeholder="Tìm kiếm sản phẩm đồ cũ..."]',
  );
  if (searchInput) {
    let typingTimer;
  ...
  const categoryFilterButtons = document.querySelectorAll(
    '.category-btn:not([data-navigate="1"])',
  );
...
function fetchProducts(
  ...
  const productGrid =
    ...
    document.querySelector(".grid-cols-2.md\\:grid-cols-4") ||
    document.querySelector(".grid");
  if (!productGrid) return;

  if (
    productGrid.getAttribute &&
    productGrid.getAttribute("data-server-rendered") === "1" &&
    !forceRefresh
  ) {
    console.debug(
      "[products.js] Server already rendered products; starting poll to detect new items.",
    );
    ...
    async function pollNewest() {
      try {
        ...
        const newest =
          arr[0].created_at || arr[0].Created_at || arr[0].createdAt || null;
        const currentLast =
          productGrid.getAttribute("data-last-created") || null;
        if (newest && (!currentLast || newest > currentLast)) {
          console.debug(
            "[products.js] New product detected, reloading product list.",
          );
  ...
  fetch(url)
    ...
    .then((result) => {
      ...
      const searchKeyword = (searchQuery || urlParams.get("search") || "")
        .trim()
        .toLowerCase();
      ...
      if (sortValue === "price-asc") {
        ...
        // Mặc định mới nhất: sắp xếp theo created_at giảm dần
        products.sort((a, b) => {
          const dateA = new Date(a.created_at ?? a.Created_at ?? a.createdAt ?? 0);
          const dateB = new Date(b.created_at ?? b.Created_at ?? b.createdAt ?? 0);
          return dateB - dateA;
        });
      }
      ...
      products.forEach((product) => {
        ...
        const productImage = imageField
          ? imageField.startsWith("http://") ||
            ...
            ? imageField
            : `/Project-Web-Programming/backend/uploads/products/${imageField}`
          : "";
        const location =
          product.Location || product.location || "Quận 1, TP. HCM";
    ...
    .then((result) => {
      ...
      if (document.getElementById("product-category"))
        document.getElementById("product-category").innerText =
          product.danh_muc || "Đồ cũ";
      if (document.getElementById("product-seller"))
        document.getElementById("product-seller").innerText =
          product.nguoi_ban || "Thành viên Chợ Cũ";
...
function switchSellerTab(status) {
  ...
  fetch(
    `/Project-Web-Programming/backend/public/index.php/api/products?status=${status}`,
  )
    .then((response) => response.json())
    .then((result) => {
      ...
      container.innerHTML = products
        .map((p) => {
          ...
          const imgSrc = imgVal
            ? imgVal.startsWith("http://") || imgVal.startsWith("https://")
              ? imgVal
              : `/Project-Web-Programming/backend/uploads/products/${imgVal}`
            : "";
...
async function deleteProduct(id) {
  if (
    await showConfirm(
      "Xóa tin",
      "Bạn có chắc chắn muốn xóa vĩnh viễn tin thanh lý này không?",
    )
  ) {
    try {
      let res = await fetch(
        "/Project-Web-Programming/backend/public/index.php/api/products/delete",
        {
          method: "POST",
          headers: {
      ...
      if (res.ok && !data.error) {
        ...
        const activeTabBtn = document.querySelector(
          ".seller-tab-btn.text-blue-600",
        );
        const activeTab = activeTabBtn
          ? activeTabBtn.id.replace("tab-btn-", "")
          : "available";
      ...
      showAlert(
        "Lỗi hệ thống",
        "Lỗi kết nối khi gửi yêu cầu xóa sản phẩm.",
        "error",
      );
    }
  }
  ...
  fetch("/Project-Web-Programming/backend/public/index.php/api/seller/stats")
    ...
    .then((data) => {
      if (data && !data.error) {
        ...
        if (revEl) {
          revEl.innerText = new Intl.NumberFormat("vi-VN", {
            style: "currency",
            currency: "VND",
